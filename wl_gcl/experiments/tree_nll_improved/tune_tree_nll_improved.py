...
from wl_gcl.experiments.tree_nll_improved.train_tree_nll_improved import (
    TreeNLLImprovedConfig,
    make_tree_nll_improved_cfg,
    train_tree_nll_improved,
)
...
PRESET_OVERRIDES = {
    # Reproduces previous Tree-NLL behavior (for ablation reference)
    "baseline": {
        "branch_only_tree_nll": False,
        "contrastive_mode": "legacy",
        "nce_level_policy": "deepest",
    },
    # Only branch-only Tree-NLL, no contrastive fix
    "branch_only": {
        "branch_only_tree_nll": True,
        "contrastive_mode": "legacy",
        "nce_level_policy": "deepest",
    },
    # Branch-only + robust contrastive mask, same deep level policy
    "branch_only_fixed": {
        "branch_only_tree_nll": True,
        "contrastive_mode": "fixed",
        "nce_level_policy": "deepest",
    },
    # Branch-only + robust contrastive + choose most branching active level
    "branch_only_fixed_maxbranch": {
        "branch_only_tree_nll": True,
        "contrastive_mode": "fixed",
        "nce_level_policy": "max_branching",
    },
}
...
def sample_unique_random_candidates(
    ...
    if len(out) < n_trials:
        print(
            f"[WARN] Sampled only {len(out)}/{n_trials} new trials after {attempts} attempts."
        )

    return out


def load_reference_best(dataset: str, method: str) -> float | None:
    # method in {'wl','wl_hierarchy'}
    fp = Path("runs") / "tune" / method / dataset / "best.json"
    if not fp.exists():
        return None
...
def load_prior_wl_hierarchy_cfg(dataset: str) -> Dict[str, Any] | None:
    ...
    try:
        ...
        if not isinstance(prior_cfg, dict):
            return None
        valid_fields = {f.name for f in dataclasses.fields(TreeNLLImprovedConfig)}
        return {k: v for k, v in prior_cfg.items() if k in valid_fields}
    except Exception:
        return None


def run_trial(cfg: TreeNLLImprovedConfig, trial_seed: int) -> Dict[str, Any]:
    set_seed(trial_seed)
    t0 = time.time()
    metrics = train_tree_nll_improved(cfg)
    dt = time.time() - t0
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Tune improved Tree-NLL pipeline with ablation presets."
    )
    parser.add_argument("--datasets", type=str, default="all")
    parser.add_argument("--model", type=str, default="gin", choices=["gin", "gcn", "gat", "wlhn"])
    parser.add_argument("--preset", type=str, default="branch_only_fixed_maxbranch", choices=sorted(PRESET_OVERRIDES.keys()))
    ...
    parser.add_argument("--out_dir", type=str, default="runs/tune_tree_nll_improved")
    parser.add_argument("--save_best_encoder", action="store_true")
    parser.add_argument("--skip_prior_best", action="store_true")
    ...
    preset_overrides = PRESET_OVERRIDES[args.preset]

    summary_rows: List[Dict[str, Any]] = []

    for dsi, dataset in enumerate(datasets):
        print("\n" + "=" * 80)
        print(f"[DATASET {dsi + 1}/{len(datasets)}] {dataset} | preset={args.preset}")
        print("=" * 80)

        out_dir = Path(args.out_dir) / args.preset / dataset
        ...
        def make_trial_cfg(params: Dict[str, Any]) -> TreeNLLImprovedConfig:
            cfg = make_tree_nll_improved_cfg(dataset)
            cfg = apply_params(cfg, params)
            cfg = apply_params(cfg, preset_overrides)
            cfg = apply_params(
                cfg,
                {
                    "model": args.model,
                    ...
                    "output_dir": str(Path(args.out_dir) / "checkpoints" / args.preset),
                },
            )
            ...
            summary_rows.append(
                {
                    ...
                    "wl_gin_best_accuracy": load_reference_best(dataset, "wl"),
                    "wl_hierarchy_ce_best_accuracy": load_reference_best(dataset, "wl_hierarchy"),
                    "tree_nll_improved_best_accuracy": None,
                }
            )
            continue
        ...
        summary_rows.append(
            {
                ...
                "wl_gin_best_accuracy": load_reference_best(dataset, "wl"),
                "wl_hierarchy_ce_best_accuracy": load_reference_best(dataset, "wl_hierarchy"),
                "tree_nll_improved_best_accuracy": best_acc,
            }
        )

        print(f"[DONE {dataset}] best_accuracy={best_acc:.4f} trial={best['trial_id']}")

    summary_fp = Path(args.out_dir) / args.preset / "summary_tree_nll_improved.json"
    ...
    print("=" * 80)
    print("Dataset | WL-GIN | WL-Hierarchy (CE) | Tree-NLL Improved")
    print("---|---:|---:|---:")
    for row in summary_rows:
        print(
            f"{row['display']} | {fmt_pct(row['wl_gin_best_accuracy'])} | "
            f"{fmt_pct(row['wl_hierarchy_ce_best_accuracy'])} | "
            f"{fmt_pct(row['tree_nll_improved_best_accuracy'])}"
        )
