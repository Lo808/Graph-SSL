...
from wl_gcl.src.losses.wl_losses import hierarchy_regularization
from wl_gcl.src.trainers.eval import evaluate_linear_probe

from wl_gcl.experiments.tree_nll_improved.losses import (
    tree_nll_loss_branch_only,
    compute_wl_contrastive,
    branching_fraction_for_level,
)
...
class TreeNLLImprovedConfig:
    ...
    branch_only_tree_nll: bool = True
    branch_min_children: int = 2
    contrastive_mode: str = "fixed"  # {'legacy','fixed'}
    nce_level_policy: str = "max_branching"  # {'deepest','middle','max_branching'}

    save_best: bool = True
    output_dir: str = "runs/wl_hierarchy_tree_nll_improved"


def make_tree_nll_improved_cfg(dataset: str) -> TreeNLLImprovedConfig:
    base = TreeNLLImprovedConfig(dataset=dataset)
...
def _select_nce_level(active_levels: list[int], policy: str, branch_score: Dict[int, float]) -> int:
    if not active_levels:
        raise ValueError("active_levels is empty")

    p = policy.lower()
    if p == "deepest":
        return max(active_levels)
    if p == "middle":
        return active_levels[len(active_levels) // 2]
    if p == "max_branching":
        return max(active_levels, key=lambda t: branch_score.get(t, 0.0))
    raise ValueError(f"Unknown nce level policy: {policy}")


def train_tree_nll_improved(cfg: TreeNLLImprovedConfig) -> Dict[str, Any]:
    ...
    branch_score = {t: branching_fraction_for_level(engine, t) for t in sorted_levels}
    ...
    print(
        f"\n[WL-HIERARCHY-TREE-NLL-IMPROVED] Dataset={cfg.dataset.upper()} | "
        f"Model={cfg.model.upper()} | epochs={cfg.epochs} | device={cfg.device}\n"
        f"  branch_only={cfg.branch_only_tree_nll} (min_children={cfg.branch_min_children}) | "
        f"contrastive_mode={cfg.contrastive_mode} | nce_level_policy={cfg.nce_level_policy}\n"
    )
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        loss_tree, tree_stats = tree_nll_loss_branch_only(
            engine,
            logits,
            active_levels,
            branch_only=cfg.branch_only_tree_nll,
            min_children=cfg.branch_min_children,
        )
        ...
        t_contrast = _select_nce_level(active_levels, cfg.nce_level_policy, branch_score)
        loss_nce, nce_stats = compute_wl_contrastive(
            engine,
            z,
            level=t_contrast,
            temperature=cfg.temperature,
            mode=cfg.contrastive_mode,
        )
        ...
        if epoch % cfg.log_interval == 0 or epoch == cfg.epochs:
            ...
            print(
                ...
                f"Acc: {acc:.4f} "
                f"LvNCE={t_contrast} "
                f"TreeNodes={tree_stats.get('used_nodes', 0)} "
                f"NCEValid={nce_stats.get('valid_rows', 'na')}"
            )

    print(
        f"[TREE-NLL-IMP | {cfg.dataset.upper():<12}] "
    ...
    return {
        ...
        "branch_score": {int(k): float(v) for k, v in branch_score.items()},
